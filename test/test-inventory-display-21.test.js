// test-inventory-display test suite - part 21
// Test coverage for SNF - Simple NFT Mint

const { describe, it, expect, beforeEach, afterEach } = require('vitest');

describe('test-inventory-display - part 21', () => {
  let ctx;
  ...
  it('should enforce max supply boundary - case 21', () => {
    const tokenId = Math.min(21 * 500, ctx.maxSupply);
    expect(tokenId).toBeLessThanOrEqual(ctx.maxSupply);
    expect(tokenId).toBeGreaterThan(0);
  });

  it('should calculate net proceeds correctly - variant 21', () => {
    const salePrice = 21 * 100000;
    const netProceeds = salePrice - ctx.saleFee;
    expect(netProceeds).toBe(salePrice - 1300);
    expect(netProceeds).toBeGreaterThan(0);
  });

  it('should format address display - scenario 21', () => {
  ...
  it('should handle edge case for commit index 21', () => {
    const edgeValue = 21 === 1 ? 'first' : 21 === 21 ? 'last' : 'middle';
